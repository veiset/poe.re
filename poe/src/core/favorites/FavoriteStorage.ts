...
export const listFavorites = (profileName: string): FavoriteRecord[] =>
  parseFavoriteRecords(loadSettings(profileName).favorites);
export const getFavorite = (
  profileName: string,
  id: string,
): FavoriteRecord | undefined =>
  listFavorites(profileName).find((favorite) => favorite.id === id);
...
export const createFavorite = (
  ...
  if (!record.name || !record.regex.trim())
    throw new Error("A favorite needs a name and a non-empty regex");
  updateSettings(profileName, (settings) => ({
    ...settings,
    favorites: [...parseFavoriteRecords(settings.favorites), record],
  }));
  return record;
};

export const createStaticFavorite = (
  profileName: string,
  regex: string,
  metadata: FavoriteMetadata,
): StaticFavoriteRecord => {
  ...
  if (!record.name || !record.regex)
    throw new Error("A static favorite needs a name and a non-empty regex");
  updateSettings(profileName, (settings) => ({
    ...settings,
    favorites: [...parseFavoriteRecords(settings.favorites), record],
  }));
...
export const createFavoriteGroup = (
  profileName: string,
  ids: readonly string[],
  ...
  const now = new Date().toISOString();
  const record: FavoriteGroupRecord = {
    schemaVersion: 1,
    kind: "group",
    id: createFavoriteId(),
    name: metadata.name.trim(),
...
export const setFavoriteHidden = (
  profileName: string,
  id: string,
  hidden: boolean,
) =>
  updateSettings(profileName, (settings) => ({
    ...settings,
    favorites: parseFavoriteRecords(settings.favorites).map((favorite) =>
      favorite.id === id
        ? { ...favorite, hidden: hidden || undefined }
        : favorite,
    ),
  }));
export const updateFavoriteGroup = (
  profileName: string,
  id: string,
  ids: readonly string[],
  operator: FavoriteGroupOperator,
  ...
  updateSettings(profileName, (settings) => {
    ...
    if (!target || !isFavoriteGroup(target))
      throw new Error("Favorite group not found.");
    const members = selectValidFavoriteGroupMembers(
      favorites.filter(isGroupableFavorite),
      ids,
      operator,
    );
    return {
...
export const updateFavoriteMetadata = (
  profileName: string,
  id: string,
  metadata: FavoriteMetadata,
): FavoriteRecord => {
...
export const updateFavoriteSnapshot = (
  profileName: string,
  id: string,
  snapshot: FavoriteSnapshot,
): FavoriteRecord => {
...
export const updateStaticFavorite = (
  profileName: string,
  id: string,
  regex: string,
  metadata: FavoriteMetadata,
  ...
  if (!name || !normalizedRegex)
    throw new Error("A static favorite needs a name and a non-empty regex");
  updateSettings(profileName, (settings) => {
    const favorites = parseFavoriteRecords(settings.favorites);
    const target = favorites.find((favorite) => favorite.id === id);
    if (!target || !isStaticFavorite(target))
      throw new Error("Static favorite not found.");
    return {
      ...settings,
      favorites: favorites.map((favorite) => {
        if (favorite.id !== id) return favorite;
...
const duplicateName = (
  ...
  const names = new Set(
    favorites.map((favorite) => favorite.name.toLocaleLowerCase()),
  );
...
export const duplicateFavorite = (
  ...
  updateSettings(profileName, (settings) => {
    ...
    duplicate = {
      ...
      createdAt: now,
      updatedAt: now,
      ...(isRegexFavorite(source)
        ? {
            configuration: cloneFavoriteConfiguration(source.configuration),
    ...
    return {
      ...
      favorites: favorites.flatMap((favorite) =>
        favorite.id === id ? [favorite, duplicate!] : [favorite],
      ),
    };
  });
...
export const removeFavorite = (profileName: string, id: string): void => {
  updateSettings(profileName, (settings) => {
    const remaining = parseFavoriteRecords(settings.favorites)
      .filter((favorite) => favorite.id !== id)
      .flatMap<FavoriteRecord>((favorite) => {
        if (!isFavoriteGroup(favorite)) return [favorite];
        const memberIds = favorite.memberIds.filter(
          (memberId) => memberId !== id,
        );
        return memberIds.length >= 2
          ? [{ ...favorite, memberIds, updatedAt: new Date().toISOString() }]
          : [];
      });
...
export const reorderFavorites = (
  ...
  updateSettings(profileName, (settings) => {
    const byId = new Map(
      parseFavoriteRecords(settings.favorites).map((favorite) => [
        favorite.id,
        favorite,
      ]),
    );

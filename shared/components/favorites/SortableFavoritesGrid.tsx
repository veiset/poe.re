...
import {
  FavoriteGroupResolution,
  favoriteGroupIssueText,
...
export const SortableFavoritesGrid = <T extends SortableFavoriteView>({
  favorites,
  visible,
  copiedFavoriteId,
  visibilityMode,
  ...
  const dragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (from >= 0 && to >= 0)
      onReorder(arrayMove(favorites, from, to).map((favorite) => favorite.id));
  };
  ...
  return (
    <DndContext
      ...
      modifiers={[restrictToViewportEdges]}
      onDragEnd={dragEnd}
    >
      <SortableContext
        items={visible.map((favorite) => favorite.id)}
      ...
      >
        <div className="favorites-grid">
          {visible.map((favorite) => {
            ...
            const groupError =
              favorite.kind === "group" && favorite.groupResolution
                ? favoriteGroupIssueText(favorite.groupResolution)
                : undefined;
            const cardFavorite = {
              ...favorite,
              sourceIcon: getSourceIcon(favorite),
              iconVariant: favorite.kind === "static" ? "regex" : undefined,
              regexLength:
                favorite.kind === "group"
                  ? favorite.groupResolution?.regexLength
                  : undefined,
              error: groupError,
            };
            ...
            return (
              <FavoriteCard
                ...
                onCopied={() => onCopied(favorite.id)}
                onDetails={() => onDetails(favorite)}
                onCustomize={
                  favorite.kind === "favorite"
                    ? () => onCustomize(favorite)
                    : undefined
                }
                onEdit={() =>
                  favorite.kind === "static"
                    ? onEditStatic(favorite)
                    : onEditRegex(favorite)
                }
                canEdit={favorite.kind !== "group"}
                onEditGroup={
                  favorite.kind === "group"
                    ? () => onEditGroup(favorite)
                    : undefined
                }
                onDuplicate={() => onDuplicate(favorite)}
                visibilityMode={visibilityMode}
                hidden={favorite.hidden}
                onToggleVisibility={() => onToggleVisibility(favorite)}
                onMove={(offset) => onMove(favorite.id, offset)}

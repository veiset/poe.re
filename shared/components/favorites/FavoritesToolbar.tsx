...
export const FavoritesToolbar = ({tags, selectedTags, visibilityMode, hasFavorites, canCreateGroup, onTagsChange, onToggleVisibilityMode, onCreateGroup, onCreateStatic}: {
  ...
  return <div className={`favorites-toolbar${visibilityMode ? " favorites-toolbar-visibility" : ""}`}>
    {visibilityMode
    ...
    <div className="favorites-toolbar-actions">
      <button type="button" className={`favorites-action-visibility${visibilityMode ? " active" : ""}`} aria-pressed={visibilityMode} onClick={onToggleVisibilityMode}>{visibilityMode ? "◉ Done managing visibility" : "👁 Manage visibility"}</button>
      <button type="button" className="favorites-action-static" onClick={onCreateStatic}>Create static</button>
      <button type="button" className="favorites-action-group" disabled={!canCreateGroup} title={canCreateGroup ? "Create a favorite group" : "At least two individual favorites are required"} onClick={onCreateGroup}>Create group</button>
    </div>
  </div>;
};

...
export const FavoriteTagFilter = ({tags, selectedTags, onChange}: FavoriteTagFilterProps) => {
  ...
  const toggle = (tag: string) => onChange(selectedTags.includes(tag) ? selectedTags.filter((value) => value !== tag) : [...selectedTags, tag]);
  return <div className="favorite-tag-filter" aria-label="Filter favorites by tag">
